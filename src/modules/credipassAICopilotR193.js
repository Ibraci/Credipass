...
export function buildCopilotContext(db,d,user){
  ...
  return {
    ...
    scoreDetail:{band:d.incluscoreDetail?.band||'',strengths:d.incluscoreDetail?.strengths||[],weaknesses:d.incluscoreDetail?.weaknesses||[],outOfNorm:d.incluscoreDetail?.outOfNorm||[],missing:(d.incluscoreDetail?.missingCriteria||[]).slice(0,5)},scores:{incluscore:n(d.incluscore),confidence:n(d.confidence),financialCapacity:n(d.financial?.capacity),recommendationAmount:n(d.recommendation?.amount),installment:n(d.recommendation?.simulation?.installment),margin:n(d.recommendation?.simulation?.margin)},
    productForm:f,
...
export async function askCopilot(question,{context,preferRemote=true}={}){
  ...
  return {answer:answerCopilotOffline(question,context),mode:'ASSISTANT_LOCAL_HORS_LIGNE',provider:null};
}
