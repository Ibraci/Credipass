# TEST PHYSIQUE R19.3.2 — ANDROID / PC / LAN / OFFLINE
...
1. Vérifier PostgreSQL central :

```powershell
npm run db:check
```

Le résultat attendu est `POSTGRESQL CENTRAL: PASS`.

2. Sur le PC agence, définir un mot de passe de démonstration non trivial :

```powershell
$env:CREDIPASS_DEMO_PASSWORD='A_DEFINIR_LOCALEMENT'
```

3. Lancer `LANCER_CREDIPASS_LAN.bat`.
4. Ouvrir `http://localhost:8092/api/health` et vérifier `database.engine = POSTGRESQL` et `terminalStorage = IndexedDB`.
5. Noter l'adresse LAN affichée par le serveur, par exemple `http://192.168.x.x:8092`.
6. Vérifier le pare-feu Windows et autoriser Node.js uniquement sur le réseau privé utilisé pour le test.
7. Ouvrir CREDIPASS sur le PC et sur un second PC/Android du même réseau.
