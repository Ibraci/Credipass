...
function createPostgresStore(sql){
  ...
  const api={
    ...
    async updateUserProfile(login,{displayName,phone,email}){await exec('UPDATE users SET display_name=$1,phone=$2,email=$3 WHERE login=$4',[displayName||null,phone||null,email||null,login])},
    async getSessionUser(token,now){return one('SELECT s.login,u.role,u.name,u.agency,u.scope_mode,u.display_name,u.phone,u.email FROM sessions s JOIN users u ON u.login=s.login WHERE s.token=$1 AND s.expires_at>$2',[token,now])},
...
function createMemoryStore(){
  ...
  const api={
    ...
    async upsertUser({login,passwordHash,salt,role,name,agency,scopeMode}){const old=users.get(login);users.set(login,{login,password_hash:old?.password_hash||passwordHash,salt:old?.salt||salt,role,name,agency,scope_mode:scopeMode,failed_attempts:old?.failed_attempts||0,locked_until:old?.locked_until||0,display_name:old?.display_name||null,phone:old?.phone||null,email:old?.email||null})},
    ...
    async getSessionUser(token,now){const s=sessions.get(token);if(!s||s.expiresAt<=now)return null;const u=users.get(s.login);return u?{login:u.login,role:u.role,name:u.name,agency:u.agency,scope_mode:u.scope_mode,display_name:u.display_name,phone:u.phone,email:u.email}:null},
async updateUserProfile(login,{displayName,phone,email}){const u=users.get(login);if(u)Object.assign(u,{display_name:displayName||null,phone:phone||null,email:email||null})},
